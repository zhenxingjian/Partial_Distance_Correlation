...
class StackedTrainer():
    def __init__(self, model_type, scheduler, input_shape, num_classes, **kwargs):
        ...
        for _ in range(args.num_nets):
            self._make_model(model_type, num_classes, (None, *input_shape))
    ...
    def predict(self, pred_ds, best, net_idx):
        ...
        print ('Prediction Accuracy: {:.2f}%'.format(self.test_acc.result()*100))



    def _make_model(self, model_type, num_classes, input_shape):  
        if 'lenet' in model_type:
            self.models += [LeNet(num_classes)]
        elif 'alexnet' in model_type:
            self.models += [AlexNet(num_classes)]
        elif 'vgg' in model_type:
            self.models += [VGG(model_type, num_classes)]
        elif 'resnet' in model_type:
            if 'se' in model_type:
                if 'preact' in model_type:
                    self.models += [SEPreActResNet(model_type, num_classes)]
                else:
                    self.models += [SEResNet(model_type, num_classes)]
            else:
                if 'preact' in model_type:
                    self.models += [PreActResNet(model_type, num_classes)]
                else:
                    self.models += [ResNet(model_type, num_classes, input_shape, return_feats=not args.debug)]
        elif 'densenet' in model_type:
            self.models += [DenseNet(model_type, num_classes)]
        elif 'mobilenet' in model_type:
            if 'v2' not in model_type:
                self.models += [MobileNet(num_classes)]
            else:
                self.models += [MobileNetV2(num_classes)]
        else:
